...
const ShopResults = ({ products, itemsPerPage }) => {
...
const ShopPage = () => {
  ...
  useEffect(() => {
    // Fetch without filter; apply case-insensitive filtering on the client.
  ...
  const filteredProducts = normalizedFilter
    ...
    : shopProducts;
  const displayTotal = normalizedFilter ? filteredProducts.length : total;
...
"
      ...
      <div className="flex flex-col justify-between gap-y-6 max-w-7xl mx-auto items-center mt-12 md:flex md:flex-row md:max-w-262.5 md:mx-20">
        <h3 className="text-second-text text-sm font-bold">
          Showing all {displayTotal} Results
        </h3>
      ...
      <ShopResults
        key={`${categoryId || "all"}-${sort}-${debouncedFilter}`}
